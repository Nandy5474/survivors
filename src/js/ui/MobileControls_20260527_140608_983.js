...
export default class MobileControls {
  ...
  /**
   * 设置操作模式
   * @param {'exploration'|'combat'|'base'} mode
   */
  setMode(mode) {
    ...
    // 摇杆在探索和战斗模式都显示
    if (this._joystickEl) this._joystickEl.style.display = (isExplore || isCombat) ? 'block' : 'none';
    // 交互按钮仅在探索模式且可交互时显示（由 setInteractButtonVisible 控制）
  ...
  _createStyles() {
    ...
    s.textContent = `
      #mc-joystick{
        ...
        bottom:env(safe-area-inset-bottom, 110px);
      ...
      #mc-attack-btn{
        right:env(safe-area-inset-right, 24px);
        bottom:env(safe-area-inset-bottom, 200px);
        background:rgba(220,38,38,0.5);
        border:2px solid rgba(220,38,38,0.7);
      }
      #mc-dodge-btn{
        right:env(safe-area-inset-right, 90px);
        bottom:env(safe-area-inset-bottom, 200px);
      ...
      #mc-backpack-btn{
        right:env(safe-area-inset-right, 24px);
        bottom:env(safe-area-inset-bottom, 130px);
  ...
  _bindTouchEvents() {
    const onStart = (e) => {
      for (let i = 0; i < e.changedTouches.length; i++) {
        ...
        // 左半屏激活摇杆（更宽松的激活区域）
        if (t.clientX < window.innerWidth * 0.55 && !this._activeTouch.active) {
          e.preventDefault();
          this._activeTouch.active = true;
          this._activeTouch.id = t.identifier;
          this._positionJoystickBase(t.clientX, t.clientY);
          // 显示摇杆（确保可见）
          if (this._joystickEl) this._joystickEl.style.display = 'block';
        }
      }
    };
    const onMove = (e) => {
  ...
  _removeTouchEvents() {
    for (const fn of this._touchCleanups) fn();
    this._touchCleanups = [];
  }
  _unbindSceneEvents() {
  ...
  _positionJoystickBase(x, y) {
    ...
    // 将摇杆底座定位到触摸点附近（限制偏移范围）
    const elR = this._joystickEl.getBoundingClientRect();
    const cx = elR.left + elR.width / 2;
    const cy = elR.top + elR.height / 2;
    const ox = Math.max(-20, Math.min(20, x - cx));
    const oy = Math.max(-20, Math.min(20, y - cy));
